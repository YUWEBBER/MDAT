...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        self.tb = SummaryWriter(os.path.join(opt.checkpoints_dir, opt.name))
    ...
    def print_current_losses(self, epoch, iters, losses, t_comp, global_step=None):
        ...
        for k, v in losses.items():
            message += '%s:%.3f \t' % (k, v)
            loss += v
            tb.add_scalar('Train/'+k, v, global_step=global_step)
        message += 'Loss: %.3f ' % (loss)
        tb.add_scalar('Train/Loss', loss, global_step=global_step)
